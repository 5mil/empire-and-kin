...
pub fn drawStars(gfx: backend.Backend, wanted: u8, x: i32, y: i32) void {
    ...
    gfx.drawText(buf[0..5], x + 70, y, col);
}

pub fn drawPoliceBanner(gfx: backend.Backend, police: living.PoliceState, chase: action.ChaseState, y: i32) void {
    var buf: [96]u8 = undefined;
    if (chase.active) {
        const line = std.fmt.bufPrint(&buf, "!! PURSUIT !!  distance {d:.0}  heat {d}  — floor it or hide", .{ chase.distance, chase.pursuit_heat }) catch "PURSUIT";
        gfx.drawText(line, 10, y, backend.Color.rgb(255, 60, 50));
    } else if (chase.caught) {
        gfx.drawText("BUSTED — heat stays high", 10, y, backend.Color.rgb(255, 80, 60));
    } else if (chase.escaped) {
        gfx.drawText("Escaped the heat… for now", 10, y, backend.Color.rgb(120, 200, 120));
    } else if (police.alert_level >= 2) {
        const line = std.fmt.bufPrint(&buf, "Police: {s}  |  units nearby ~{d}", .{ living.policeStatus(police), police.units_nearby }) catch "";
