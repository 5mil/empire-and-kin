...
fn initImpl(title: []const u8, width: u32, height: u32) !void {
    ...
    mapper = .{};
    text_count = 0;
    std.debug.print("[GLBackend] OpenGL 3.3 context ready {d}x{d}\n", .{ width, height });
}
...
pub fn pollRawKeys() input.RawKeys {
    ...
    if (window) |*w| {
        const kc = glfw_window.c;
        raw.w = w.keyDown(kc.GLFW_KEY_W);
        raw.a = w.keyDown(kc.GLFW_KEY_A);
        raw.s = w.keyDown(kc.GLFW_KEY_S);
        raw.d = w.keyDown(kc.GLFW_KEY_D);
        raw.e = w.keyDown(kc.GLFW_KEY_E);
        raw.f = w.keyDown(kc.GLFW_KEY_F);
        raw.q = w.keyDown(kc.GLFW_KEY_Q);
        raw.r = w.keyDown(kc.GLFW_KEY_R);
        raw.h = w.keyDown(kc.GLFW_KEY_H);
        raw.x = w.keyDown(kc.GLFW_KEY_X);
        raw.tab = w.keyDown(kc.GLFW_KEY_TAB);
        raw.enter = w.keyDown(kc.GLFW_KEY_ENTER);
        raw.escape = w.keyDown(kc.GLFW_KEY_ESCAPE);
        raw.space = w.keyDown(kc.GLFW_KEY_SPACE);
        raw.key_1 = w.keyDown(kc.GLFW_KEY_1);
        raw.key_2 = w.keyDown(kc.GLFW_KEY_2);
        raw.key_3 = w.keyDown(kc.GLFW_KEY_3);
        raw.key_4 = w.keyDown(kc.GLFW_KEY_4);
        raw.key_5 = w.keyDown(kc.GLFW_KEY_5);
        raw.f5 = w.keyDown(kc.GLFW_KEY_F5);
        raw.f9 = w.keyDown(kc.GLFW_KEY_F9);
    }
    return raw;
}
