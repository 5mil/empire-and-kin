...
/// Safehouse entrance (world x,z) — E to use when near.
...
fn jobBeacon(gfx: backend.Backend, x: f32, z: f32, near: bool, active: bool) void {
    ...
    if (near) {
        box(gfx, x, 4.4, z, 1.4, 0.35, 1.4, backend.Color.rgb(255, 240, 120));
    }
}
...
fn parkedCar(gfx: backend.Backend, x: f32, z: f32, yaw_deg: f32, col: backend.Color) void {
    _ = yaw_deg;
...
fn safehouse(gfx: backend.Backend, lit: bool) void {
    // Distinct green-door social club
...
pub fn drawMinimalScene(
    ...
    box(gfx, 6, 0.05, 20, 2.0, 0.04, 0.25, backend.Color.rgb(180, 160, 60));
    box(gfx, 18, 0.05, 20, 2.0, 0.04, 0.25, backend.Color.rgb(180, 160, 60));
    ...
    // Parked cars
    parkedCar(gfx, 5.5, 22.5, 0, backend.Color.rgb(50, 55, 70));
    parkedCar(gfx, 18.5, 17.5, 0, backend.Color.rgb(90, 40, 35));
    parkedCar(gfx, 8.0, 15.0, 0, backend.Color.rgb(35, 45, 40));
