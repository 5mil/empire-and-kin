//! Procedural humanoid — maximum box fidelity + walk bob toward Sims readability.
//! Swap for Quaternius GLB when assets/cc0/characters is populated.
...
/// time_s: seconds; moving: any horizontal motion this frame.
pub fn drawBoss(gfx: backend.Backend, x: f32, z: f32, time_s: f32, moving: bool) void {
    ...
    // Legs with walk offset on Z
    gfx.drawBox(.{ .x = x - 0.15, .y = 0.55 + bob, .z = z + ll }, 0.2, 0.7, 0.26, pants);
    gfx.drawBox(.{ .x = x + 0.15, .y = 0.55 + bob, .z = z + rl }, 0.2, 0.7, 0.26, pants);
    gfx.drawBox(.{ .x = x - 0.15, .y = 0.22 + bob, .z = z + 0.02 + ll }, 0.18, 0.35, 0.24, pants);
    gfx.drawBox(.{ .x = x + 0.15, .y = 0.22 + bob, .z = z + 0.02 + rl }, 0.18, 0.35, 0.24, pants);
    gfx.drawBox(.{ .x = x - 0.15, .y = 0.07 + bob, .z = z + 0.08 + ll }, 0.22, 0.12, 0.38, shoe);
    gfx.drawBox(.{ .x = x + 0.15, .y = 0.07 + bob, .z = z + 0.08 + rl }, 0.22, 0.12, 0.38, shoe);

    gfx.drawBox(.{ .x = x, .y = 0.95 + bob, .z = z }, 0.52, 0.18, 0.32, belt);
    gfx.drawBox(.{ .x = x, .y = 1.4 + bob, .z = z }, 0.58, 0.75, 0.36, suit);
    gfx.drawBox(.{ .x = x, .y = 1.55 + bob, .z = z + 0.12 }, 0.28, 0.2, 0.08, shirt);
    gfx.drawBox(.{ .x = x - 0.42, .y = 1.65 + bob, .z = z }, 0.22, 0.22, 0.28, suit);
    gfx.drawBox(.{ .x = x + 0.42, .y = 1.65 + bob, .z = z }, 0.22, 0.22, 0.28, suit);

    gfx.drawBox(.{ .x = x - 0.48, .y = 1.35 + bob, .z = z + la }, 0.15, 0.4, 0.16, suit);
    gfx.drawBox(.{ .x = x + 0.48, .y = 1.35 + bob, .z = z + ra }, 0.15, 0.4, 0.16, suit);
    gfx.drawBox(.{ .x = x - 0.5, .y = 1.05 + bob, .z = z + la }, 0.14, 0.32, 0.14, suit);
    gfx.drawBox(.{ .x = x + 0.5, .y = 1.05 + bob, .z = z + ra }, 0.14, 0.32, 0.14, suit);
    gfx.drawBox(.{ .x = x - 0.5, .y = 0.82 + bob, .z = z + la }, 0.13, 0.12, 0.13, skin);
    gfx.drawBox(.{ .x = x + 0.5, .y = 0.82 + bob, .z = z + ra }, 0.13, 0.12, 0.13, skin);

    gfx.drawBox(.{ .x = x, .y = 1.88 + bob, .z = z }, 0.16, 0.16, 0.16, skin);
    gfx.drawBox(.{ .x = x, .y = 2.18 + bob, .z = z }, 0.4, 0.46, 0.38, skin);
    gfx.drawBox(.{ .x = x - 0.24, .y = 2.18 + bob, .z = z }, 0.08, 0.12, 0.1, skin);
    gfx.drawBox(.{ .x = x + 0.24, .y = 2.18 + bob, .z = z }, 0.08, 0.12, 0.1, skin);
    gfx.drawBox(.{ .x = x, .y = 2.42 + bob, .z = z }, 0.42, 0.12, 0.4, hair);
    gfx.drawBox(.{ .x = x, .y = 2.28 + bob, .z = z - 0.12 }, 0.4, 0.2, 0.12, hair);
    gfx.drawBox(.{ .x = x - 0.1, .y = 2.22 + bob, .z = z + 0.18 }, 0.06, 0.05, 0.04, backend.Color.rgb(20, 20, 25));
    gfx.drawBox(.{ .x = x + 0.1, .y = 2.22 + bob, .z = z + 0.18 }, 0.06, 0.05, 0.04, backend.Color.rgb(20, 20, 25));

    drawPlumbob(gfx, x, z, 2.72 + bob);
}

/// Back-compat: static pose.
